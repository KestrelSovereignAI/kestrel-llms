...
class LlamaCppAdapter(LLMAdapter):
    ...
    def create_provider(cls, config: Dict[str, Any]) -> ProviderInfo:
        base_url = config.get("base_url") or os.environ.get("LLAMA_CPP_BASE_URL") or cls.default_base_url
        api_key = config.get("api_key") or os.environ.get(config.get("api_key_env") or "LLAMA_CPP_API_KEY") or "local"
    ...
    async def list_models(self, client: openai.AsyncOpenAI) -> List[ModelInfo]:
        ...
        return [ModelInfo(id=item.id, provider="llama_cpp", display_name=item.id, category=ModelCategory.CHAT, supports_tools=False, supports_streaming=True) for item in getattr(models, "data", []) if getattr(item, "id", None)]
...
def _completion_kwargs(format: Optional[str], tools: Optional[List[Dict[str, Any]]], response_format: Optional[Type[BaseModel]], kwargs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in ("temperature", "top_p", "frequency_penalty", "presence_penalty", "extra_body"):
        if key in kwargs:
