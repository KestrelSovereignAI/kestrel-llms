...
class KimiAdapter(LLMAdapter):
    ...
    def create_provider(cls, config: Dict[str, Any]) -> ProviderInfo:
        ...
        return ProviderInfo(
            ...
            is_local=False,
            base_url=base_url,
            selection_hints=list(config.get("selection_hints") or ["long-context", "agentic-coding"]),
        )
    ...
    async def list_models(self, client: openai.AsyncOpenAI) -> List[ModelInfo]:
        ...
        return [ModelInfo(id=item.id, provider="kimi", display_name=item.id, category=ModelCategory.CHAT, supports_tools=True, supports_streaming=True) for item in getattr(models, "data", []) if getattr(item, "id", None)]
